...
import { ContentData, ContentLanguage } from '../../types'
...
export default async function HomePage({ params }: HomePageProps) {
  ...
  // جلب المحتوى من API أو ملف البيانات
  const contentData: ContentData = getContent()
  const currentContent: ContentLanguage = contentData[locale as keyof ContentData] || contentData.en
    ...
    <div>
      ...
      <section className="relative h-screen flex items-center justify-center bg-gradient-to-br from-secondary-900 via-secondary-800 to-secondary-900">
        ...
        <div className="absolute inset-0">
          <Image
            src={currentContent.homepage?.hero?.backgroundImage || "/images/hero-marble.jpg"}
            alt="Alhot Marble Hero"
            fill
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight">
            {currentContent.homepage?.hero?.title || 'Welcome'}
          </h1>
          <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto text-gray-200">
            {currentContent.homepage?.hero?.subtitle || 'Discover our premium marble collection'}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href={`/${locale}/products`}>
              <Button size="lg" className="text-lg px-8 py-3 group transition-all duration-300 hover:scale-105 hover:shadow-xl">
                {currentContent.homepage?.hero?.primaryButton || 'Explore Products'}
            ...
            <Link href={`/${locale}/quote`}>
              <Button variant="outline" size="lg" className="text-lg px-8 py-3 border-2 border-white text-white bg-white/10 hover:bg-white hover:text-secondary-900 font-bold shadow-lg backdrop-blur-sm group transition-all duration-300 hover:scale-105 hover:shadow-xl">
                {currentContent.homepage?.hero?.secondaryButton || 'Get Quote'}
              </Button>
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {(currentContent.homepage?.stats?.items || []).map((stat, index: number) => (
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {(currentContent.homepage?.categories?.items || []).map((category, index: number) => (
              <Link
                key={index}
                href={category.href || `/${locale}/products`}
              ...
              >
                <div className="aspect-square relative">
                  <Image
                    src={category.image || "/images/marble-category.jpg"}
                    alt={category.name || category.nameEn || 'Category'}
                    fill
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold text-center text-secondary-900 mb-12">
            {locale === 'ar' ? 'لماذا تختار الحوت ماربل؟' : 'Why Choose Alhot Marble?'}
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {(currentContent.homepage?.features?.items || []).map((feature, index: number) => {
